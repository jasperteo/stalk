...
Runtime deps (`@hono/hono`, `@valibot/valibot`, `@matmen/imagescript`, `@std/fmt`) come from **JSR**, but are declared directly in `package.json`'s `dependencies` as `npm:@jsr/<scope>__<name>` aliases (e.g. `"@hono/hono": "npm:@jsr/hono__hono@^4.12.30"`), not as `jsr:` specifiers in `deno.json`'s `imports` map — that map only holds the internal `@/` path alias. Deno resolves the `@jsr` scope natively (no `.npmrc` needed), and `deno.json`'s `preferPackageJson` makes `package.json` the dependency source of truth; `deno.lock` pins the resolved `npm.jsr.io` tarballs.
