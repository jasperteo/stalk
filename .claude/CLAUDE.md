...
- `src/log.ts` — Thin `console` wrapper (`log.info`/`success`/`warn`/`error`/`debug`) that prefixes each line with a bold, fixed-width, colored level badge via `@std/fmt/colors`; every other module logs through it so local dev and Deno Deploy output read as one leveled stream. Color is enabled only when stdout is a terminal and `NO_COLOR` is unset (`setColorEnabled` at module load), so piped/captured output stays plain text
