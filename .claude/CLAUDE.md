...
| `src/clashroyale.ts` | Battle-log fetch via the RoyaleAPI proxy, with an abort timeout. `latestBattle` picks the newest entry by cheap timestamp comparison and fully validates only that one.                                                            |
