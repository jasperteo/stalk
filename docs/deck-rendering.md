...
`concurrency(1)` — `sys` goes 0.30 → 0.15 s — and that is exactly the thread-coordination overhead
this removes.
...
dimensions `trimRaw` actually produces, which is not quite what `deno task measure` prints, so read
...
0.19%, the "about 0.2%" above.

Level 0 over level 6 buys ~27 ms per grid at a cost of ~1.86 MiB more per grid — a far more
...
superseded pipelines were reconstructed and re-measured alongside the current one — each in its own
process, median of 21 renders, compose + encode, all three under `concurrency(1)`:
...
roughly 5× the upload bytes per grid (3.28 MiB vs 0.65 MiB) — 6.55 MiB per post, since a post
...
API reference), so a post sits at 65.5% of it. Comfortable today, and `PAYLOAD_REJECTED` in
...
of the same 3.45 MiB, and the failure mode without checking first is a 400 in production that costs
...
which saves at most two renders, worth roughly 65 ms of CPU at the ~33 ms/render figure in the
