...
| `src/log.ts`         | Console wrapper (`log.info`/`success`/`warn`/`error`/`debug`) that prefixes lines with a colored, leveled badge via `@std/fmt/colors`; color only on a real terminal, so piped/Deploy output stays plain text                          |
